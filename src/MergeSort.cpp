...
//to intercept the usual calls when memory is allocated to data structures
...
vector<int> generateData(const int size) {
	...
	// uniform_int_distribution<int> dist(0, 10); // use only for proof correctness
...
vector<long long> time_experiment(vector<int>& v, string& algorithm) {
	...
	if (algorithm == "1") {
		...
		for (int i=0; i<100; i++) {
			int mergedlist;
			auto start = chrono::high_resolution_clock::now();
			mergeSort4(0, nodes.size()-1, mergedlist, nodes);
			auto end = chrono::high_resolution_clock::now();
			auto duration = chrono::duration_cast<chrono::nanoseconds>(end - start);
			times.push_back(duration.count());
